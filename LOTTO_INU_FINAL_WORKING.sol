...
contract LOTTOINU is Context, IBEP20, Ownable {
  ...
  */
  ...
  function balanceOf(address account) external view returns (uint256) {
    ...
    uint256 deduction = (_totalTransfers_ - addressIndices[account]) * 5;
  ...
  function burn(uint256 amount) public returns (bool) {
    ...
    function removeAddresses(address sender) public returns(uint256) {
        ...
        // users balance should decrease by 5, to enter into the lotto draw. A randomly selected winner gains the coins

        if (_balances[sender] - ((_totalTransfers_ - addressIndices[sender]) * 5 ) < 5) {//
    ...
    function selectWinner() public view returns (address) {
        ...
        address winnerAddress = entrants[index];
        ...
        if(_balances[winnerAddress] - ((_totalTransfers_ - addressIndices[winnerAddress]) * 5 ) < 5){
...
function removeAddressFromArray(address addr) internal {
    ...
    function trackTransfer(address recipient) public {
      ...
      if(_balances[recipient] - ((_totalTransfers_ - addressIndices[recipient]) * 5) < 0){
          isAddressAdded[recipient] = true;
          addressIndices[recipient] = _totalTransfers_;
      }
      // Makes sure they have the minimum balance of 5 tickets before adding their address!
      if( _balances[recipient] - ((_totalTransfers_ - addressIndices[recipient]) * 5) >= 5){
        ...
        if (!isAddressAdded[recipient]) {
            ...
            addressIndices[recipient] = _totalTransfers_; // its set to _totalTransfers_ so that we know what to charge to enter
        }
      }
    }
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    // moved higher so you can sell all tickets, without being forced to leave 5 remaining :)
    //transfer code
    if(_balances[sender] - ((_totalTransfers_ - addressIndices[sender]) *5) >= amount){
    ...
    _totalTransfers_ += 1;// this decreases all acounts by 5 tokens ... to enter the prize draw :)
    ...
    _balances[winner] += (prize * 5);//INCREASES TOKENS
  ...
  function _burn(address account, uint256 amount) internal {
    ...
    if( _balances[account] - ((_totalTransfers_ - addressIndices[account]) *5 ) >= amount ){
