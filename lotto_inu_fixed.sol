...
// From OpenZeppelin Contracts (last updated v5.0.0) (utils/ReentrancyGuard.sol)
...
contract LOTTOINU is Context, IBEP20, Ownable, ReentrancyGuard {
  ...
  string public _name;
  uint256 _totalTransfers_;

  // ! stop re enterant calls
  mapping(address => bool) _entrants;
  address _previous_winner_;
  uint256 private constant MAX_SUPPLY = 10 * 10**7;
  
  // !
  // Mapping to track whether an address has been added to the array
  mapping(address => bool) public isAddressAdded;
  // Array to store addresses that coins have been transferred to
  // each address is an entrant in the lotto
  address[] public entrants;
  // Mapping to track the index of each enterant
  mapping(uint256 => address) private entrants_replacer;
  uint256 entrants_number;

  // keep track of owner
  address public creator;
  // Mapping to track the index of each address in the entrants array
  mapping(address => uint256) private addressIndices;

  constructor() {
    _previous_winner_ = 0xb1Bf2555be2d8C0515505906ECAA5f3E4ce3672d; // my own address to initialise
    entrants_number = 0;
    _totalTransfers_ = 0;
    _name = "Lotto Inu";
    _symbol = "LOTTO";
  ...
  */
  ...
  function balanceOf(address account) external view override returns (uint256) {
    ...
    if (_balances[account] >= deduction) {
        return _balances[account] - deduction;
    } else {
        return 0; // Prevent negative balances
    }
  }
  ...
  function mint(uint256 amount) public onlyOwner returns (bool) {
    require(_totalSupply.add(amount) <= MAX_SUPPLY, "Exceeds maximum supply");
  ...
  function burn(uint256 amount) public returns (bool) {
    ...
    function removeAddresses(address sender) private returns(uint256) {
        ...
        //also returns the length of array of enterants
        // needs to be entrants.length because it gives the prize amount
        return entrants_number;
        
    }

    //!
    // Function to select a winner from the entrants array
    function selectWinner() private returns (address) {
        // Get a random number
        uint256 randomNumber = getRandomNumber();

        // Calculate the index of the winner using modulo operation
        uint256 index = randomNumber % entrants_number;

        // Store the address of the winner in a variable
        address winnerAddress = entrants_replacer[index];

        // Check if the winner address is still valid
        if (winnerAddress == address(0)) {// isAddressAdded[addr] == false would be better
          // The winner's entry has been deleted, so we need to burn coins
          return 0x000000000000000000000000000000000000dEaD;
        }
            
        // why check here for winner being broke? Because I tried looping but this cost a FORTUNE in gas fees.
        if(_balances[winnerAddress] - ((_totalTransfers_ - addressIndices[winnerAddress]) * 1 ) < 1){
            // burns coins if "winner is bust"
            address deadAddress = 0x000000000000000000000000000000000000dEaD;
            return deadAddress;
        }else{
        // Return the winner address
            if( _previous_winner_ == entrants_replacer[index] ){ // if someones trying to rig the lottery
              address deadAddress = 0x000000000000000000000000000000000000dEaD;
              return deadAddress;
            }else{// its fine
              _previous_winner_ = entrants_replacer[index];
              return entrants_replacer[index];// this had to go after previous line for the lotto to function correctly
            }
        }
    }

    //!
    // works well though number is large
    function getRandomNumber() private view returns (uint256) {
        // Get the hash of the most recent block
        bytes32 blockHash = blockhash(block.number - 1);
        
        // Convert the block hash to a uint256
    ...
    //!
// Internal function to remove an address from the array
function removeAddressFromArray(address addr) internal {
    // Get the index of the address
    uint256 index = addressIndices[addr];
    
    // removed address from being checked next time
    isAddressAdded[addr] = false;
    
    // Remove the address from the array
    entrants_replacer[index] = 0x000000000000000000000000000000000000dEaD;
    ...
    //!
    // Function to track addresses when coins are transferred
    function trackTransfer(address recipient) private {
      ...
      if(_balances[recipient] - ((_totalTransfers_ - addressIndices[recipient]) * 1) < 0){
          ...
          addressIndices[recipient] = entrants_number;
      }
      ...
      if( _balances[recipient] - ((_totalTransfers_ - addressIndices[recipient]) * 1) >= 1){
        ...
        if (!isAddressAdded[recipient]) {//
            entrants_replacer[entrants_number] = recipient;
            isAddressAdded[recipient] = true;
            
            // Update the index of the new address in the mapping
            addressIndices[recipient] = entrants_number; // its set to _totalTransfers_ so that we know what to charge to enter
            entrants_number += 1;
        }
      }
    }
  ...
  function _mint(address account, uint256 amount) internal {
    ...
    require( amount <= _totalSupply, "Token is fully distributed");
